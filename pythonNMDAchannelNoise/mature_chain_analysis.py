...
FRAME_DURATION = 10 # duration of single firing frame in ms


def get_bursts_of_chain_neurons(mean_burst_time, std_burst_time,
                                firing_robustness, num_soma_spikes, num_dend_spikes, filename):
    """
    Extracts burst information for neurons in the chain
    """
    (N_RA, RA_super_targets, RA_super_targets_G) = reading.read_connections(filename)
    ...
    mean_burst_time_stronglyConnected[:] = [x - min_burst_time for x in mean_burst_time_stronglyConnected]
...
def split_in_frames(burst_times, neuron_id):
    """
    Splits neurons into groups that fired in the same frame
    """
    if len(burst_times) > 0:    
        groups = [] # neurons spilt in groups based on their burst times
        burst_times_in_groups = [] # burst times of neurons split in groups
        
        first_burst_in_group = burst_times[0] # burst time of first neuron in group

        group = [] # neurons in current group
        burst_times_in_group = []
        
        for i, t in enumerate(burst_times):
            if abs(t - first_burst_in_group) <= FRAME_DURATION:
                group.append(neuron_id[i])
                burst_times_in_group.append(t)
                #print group
            else:
                groups.append(list(group))
                burst_times_in_groups.append(list(burst_times_in_group))
                
                # clear lists                
                group[:] = []
                burst_times_in_group[:] = []
                                                
                first_burst_in_group = t
        ...
        return groups, burst_times_in_groups
...
if __name__ == "__main__":
    file_chain_test = "/home/eugene/results/noDelays/replacement/sphere/matureTest/180717_lionx_1/mature_chain_test.bin"
    file_RA_super = "/home/eugene/results/noDelays/replacement/sphere/180717_lionx_1/RA_RA_super_connections_21000_.bin"
    ...
    mean_burst_time_stronglyConnected, id_stronglyConnected, firing_robustness_stronglyConnected, \
            std_burst_time_stronglyConnected, num_soma_spikes_stronglyConnected, \
            num_dend_spikes_stronglyConnected = get_bursts_of_chain_neurons(mean_burst_time, std_burst_time,
    ...
    for i in xrange(N_RA):
        if firing_robustness[i] >= ROBUST_FIRING_THRESHOLD:
            ...
            if i in id_stronglyConnected:
                print "neuron {0} is not firing robustly!".format(i)
    ...
    if len(id_robust_not_stronglyConnected) > 0:
        ...
        for i in range(len(mean_burst_time_robust)):
            if (mean_burst_time_robust[i] > 0 ):
                ...
                ax5.vlines(mean, i-0.5, i+0.5, lw=2, color=colors[i])
                ax5.hlines(i, mean - sigma, mean + sigma)
                ax5.vlines(mean-sigma, i-0.25, i+0.25)
                ax5.vlines(mean+sigma, i-0.25, i+0.25)
