...
public:
	...
	void set_targetRA(HH2_test* target, int n, double G); // set excitatory target
...
protected:
	...
	double GsKLT;	//	soma's low-threshold potassium channel conductance
	...
	double _l;	//	gating variable for soma's low-threshold potassium channel
	...
	vector<HH2_test*> _targets_RA;	//	all neuron's connections to subsequent neurons
	...
	static double lInf(double v){return 1 / (1 + exp(-(v + 40) / 5));} // was  1 / (1 + exp(-(v + 40) / 5))
	static double tauL(double v){return 10.0;}  // was 10.0
	...
	static double tauC(double v){return 15;} // original 10; wider 15.0
};
