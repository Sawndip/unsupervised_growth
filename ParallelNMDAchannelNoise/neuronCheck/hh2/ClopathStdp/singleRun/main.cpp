...
int main()
{
	...
	std::string dirname = "/home/eugene/Output/clopath/preSpikePostDoubleSpike/";
	...
	double W_TARGET_KICK = 1500;
	double t_target_kick = 110;
	
	int num_steps = 80;
	double time_kick_step = 1;
	...
	double THETA_MINUS = -75.0;
	double THETA_PLUS = -30.0;
	
	double A_LTD = 5e0; // was 14e0
	double A_LTP = 10e0; // was 8e0
	double TAU_X = 15.0;
	...
	//~ 
		...
		bool source_excited = false;
		bool target_excited = false;
		...
		{
			if ( ( static_cast<double>(t) * TIMESTEP > T_TRAINING_KICK ) && ( !source_excited ) )
			{
			...
			if ( ( static_cast<double>(t) * TIMESTEP > t_target_kick ) && ( !target_excited ) )
			{
