...
public:
	...
	void set_target(HH2_buffer* target, int ID, double G);	//	function to set a target for neuron
...
protected:
	...
	vector <HH2_buffer*> _targets;	//	vector which contains pointers to target neurons
	vector <int> _targetsID;	//	vector which contains targets' ID in layer
	vector <double> _targetsG;	//	vector which contains synaptic conductance to target neurons
