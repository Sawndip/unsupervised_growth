...
const double HHI_buffer::cm = 1.0;	// micro F / cm2
const double HHI_buffer::A = 6000;	// microns2

const double HHI_buffer::Ena = 55.0;	// mV
const double HHI_buffer::Ek = -80.0;
const double HHI_buffer::El = -65.0;
const double HHI_buffer::Ei = -75.0;

const double HHI_buffer::gNa = 100.0;	// mS/cm2
const double HHI_buffer::gKdr = 20.0;
const double HHI_buffer::gKHT = 500.0;
const double HHI_buffer::gL = 0.1;

const double HHI_buffer::tExc = 2.0;	//	ms
const double HHI_buffer::tInh = 5.0;

const double HHI_buffer::threshold = -20.0;	//	mV

const double HHI_buffer::spike_margin = 5.0; // mV

const int HHI_buffer::BUFFER_SIZE = 1000; // size of buffer for data

// noise parameters
const double HHI_buffer::G_noise = 0.45;	//	maximum noise conductance
const double HHI_buffer::lambda = 250.0; // intensity parameter for Poisson noise

HHI_buffer::HHI_buffer()
{
	// internal state
...
bool HHI_buffer::get_fired()
{
	return _fired;
}

void HHI_buffer::print_targets()
{
	printf("Connections to HVC(RA) neurons:\n");
	for (int i = 0; i < _targets.size(); i++)
	{
		printf("Target neuron %d \t Synaptic conductance %f",
			_targetsID[i], _targetsG[i]);
	}

}

vector<int> HHI_buffer::get_targetsID()
{
	return _targetsID;
}

vector<double> HHI_buffer::get_targetsG()
{
	return _targetsG;
}


int HHI_buffer::get_spike_number()
{
	return _Nspikes;
}

void HHI_buffer::set_injected_current(DDfunction f)
{
	_Iinjected = f;
	_injected_current = true;
}

void HHI_buffer::set_noise_generator(Poisson_noise* g)
{
	_generator = g;
}

void HHI_buffer::set_poisson_noise()
{
    _poisson_noise = true;
...
void HHI_buffer::set_no_poisson_noise()
{
	_poisson_noise = false;
}

void HHI_buffer::set_recording(std::string filename)
{
	_recorded = true;
	_filename = filename;
...
void HHI_buffer::set_dynamics(double tS)
{
...
void HHI_buffer::initialize_noise(double& noise_time)
{
    try
    {
...
void HHI_buffer::set_target(HH2_buffer* target, int ID, double G)
{
	_targets.push_back(target);
	_targetsID.push_back(ID);
	_targetsG.push_back(G);
}

void HHI_buffer::state_noise_check()
{
	this->state_check();
...
void HHI_buffer::noise_check(double& G, double& noise_time)
{
	if (_time > noise_time)
	{
...
void HHI_buffer::R4_step_with_target_update()
{
	if (_recorded)
		this->record_state();
...
void HHI_buffer::DP8_step_with_target_update()
{
	if (_recorded)
		this->record_state();
...
void HHI_buffer::R4_step_no_target_update()
{
	if (_recorded)
		this->record_state();
...
void HHI_buffer::DP8_step_no_target_update()
{
	if (_recorded)
		this->record_state();
...
void HHI_buffer::raiseE(double G)
{
	_Gexc += G;
}

void HHI_buffer::raiseI(double G)
{
	_Ginh += G;
}


void HHI_buffer::postsyn_update()
{
	for (unsigned i = 0; i < _targets.size(); i++)
	{
		_targets[i]->raiseI(_targetsG[i]);
	}
}

double HHI_buffer::I_total(double t)
{
...
double HHI_buffer::Ge(double t){return _Gexc * exp(-(t - _time) / tExc);}
double HHI_buffer::Gi(double t){return _Ginh * exp(-(t - _time) / tInh);}
double HHI_buffer::kV(double v, double t, double h, double w, double m3, double n4){
	return (-gL * (v - El) - gNa * h * m3 * (v - Ena) - gKdr * n4 * (v - Ek)
		- gKHT * w * (v - Ek) - Ge(t) * v - Gi(t) * (v - Ei) + 100000 * I_total(t) / A) / cm;}
double HHI_buffer::kn(double v, double n){return HHI_buffer::an(v)*(1 - n) - HHI_buffer::bn(v)*n;}
double HHI_buffer::km(double v, double m){return HHI_buffer::am(v)*(1 - m) - HHI_buffer::bm(v)*m;}
double HHI_buffer::kh(double v, double h){return HHI_buffer::ah(v)*(1 - h) - HHI_buffer::bh(v)*h;}
double HHI_buffer::kw(double v, double w){return (HHI_buffer::wInf(v) - w) / HHI_buffer::tauW(v);}
